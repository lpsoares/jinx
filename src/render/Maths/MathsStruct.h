...
const float PI_AQUI       =  3.14159265358979323846f;		// Pi

#define	DEGTORAD(x)	( ((x) * PI_AQUI) / 180.0 )
#define	RADTODEG(x)	( ((x) * 180.0) / PI_AQUI )
...
inline float ROUND(const float value, const int accuracy)		// round a float to a specified degree of accuracy
{
	...
	double accuracy2 = (double)accuracy;
	double dez = 10.0f;
	
	return(float(integer + (float(int(fraction*pow(dez,accuracy2)))) / pow(dez, accuracy2) ) );
}	// end float ROUND
