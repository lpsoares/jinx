...
void Quaternion::Slerp(const Quaternion q1, const Quaternion q2, const float slerp)
{
	...
	if (EPSILON < 1.0f + cosom)			// if both quaternions are not exactly opposite
	{
		if (EPSILON > 1.0f - cosom)		// if quaternions are close
		{
	...
	else	// quaternions are opposite
	{
		...
		// Is this half?
		//%%%%%%%%%%%%%%%
//		scale1	= sin((1.0f - slerp) * PI/2);
//		scale2	= sin(slerp * PI/2);

		scale1	= sin(PI_AQUI * (0.5 - slerp));		// calculate a perpendicular quaternion and slerp in that direction
		scale2	= sin(PI_AQUI * slerp);
